...
export default function Reviews() {
    ...
    const handleSearch = (query) => {
        ...
        const filtered = reviews.filter((review) =>
            review.bookTitle.toLowerCase().includes(query.toLowerCase())
        );
        setFilteredReviews(filtered);
    };

    const sortedByRating = [...filteredReviews].sort((a, b) => {
        if (ratingFilter === "asc") {
        ...
        <div className="w-full h-auto flex flex-col justify-center items-center mt-8">
            <div className="lg:w-full w-[80%] h-auto lg:h-[28rem] flex flex-col justify-center items-center">
                <div className="w-full">
                    <h1 className="lg:text-7xl text-5xl font-bold tracking-tighter text-center">
                        Discover,<br className="lg:hidden block" /> Review,
                        <br className="lg:hidden block" /> and Share <br />{" "}
                        <span className="text-[#d2d2d2] leading-normal lg:text-7xl text-4xl">
                            {" "}
                            Your Favorite Reads.
                        </span>
                    </h1>
                ...
                <div className="lg:w-[40%] w-[80%] relative h-[3rem] lg:h-[3rem] flex justify-center items-center rounded-full border border-gray-600 mt-8 lg:mt-10">
                    <input
                        type="text"
                        name="searchQuery"
                        className="w-[90%] h-full focus:outline-none placeholder:lg:text-sm placeholder:text-xs"
                ...
                <div className="lg:w-[40%] w-[80%] overflow-hidden overflow-x-scroll scrollbar-hide h-[3rem] flex justify-start lg:justify-center items-center mt-5">
                    <h1 className="text-md font-medium tracking-tight text-gray-700 mr-5">
                        Filter
                    </h1>
                ...
                <div className="lg:w-[90%] w-full h-auto flex lg:flex-row flex-col lg:flex-wrap justify-center items-center gap-5">
                    {sortedByDate.length > 0 ? (
                        sortedByDate.map((review) => (
                            <div
                                key={review._id}
                                className="lg:w-[30%] w-[90%] lg:h-[11rem] h-[13rem] flex-col justify-center items-center rounded-[8px] shadow-md pt-2 pr-4 pl-4"
                            >
                                <div className="w-full flex justify-between items-center">
                                    <h1 className="text-lg font-bold tracking-tighter text-black">
                                        {review.bookTitle}
                                    </h1>
                                    <small className="text-zinc-500 font-medium tracking-tighter">
                                        {new Date(
                                            review.addedDate
                                        ).toLocaleDateString()}
                                    </small>
                                </div>
                                <h1 className="text-md font-bold text-gray-500 tracking-tighter">
                                    {review.author}
                                </h1>
                                <p className="w-full h-[35%] text-sm text-justify text-gray-700 font-medium mt-2 mb-6 text-ellipsis">
                                    {review.message}
                                </p>
